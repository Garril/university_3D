...
function reducer(state = defaultState, action) {
  switch (action.type) {
    case SAVE_CLASS:
      ...
      example = data[0];
      if(example) {
        state.cur_ondata = example.ondata.toString(); // 保存当前查询日期 eg：2022-x-y
        state.cur_week = example.week.toString(); // 保存当前查询为周几 eg：周一 保存数字1
        state.cur_bid = example.bid.toString(); // 保存当前被查询建筑的bid
        key = example.bid + '-' + example.ondata; // 教学楼编号和data进行拼装，作为唯一的属性key
      }
      state.class_data[key] = data;
    ...
    case CHANGE_CUR_CLASS_LIST:
      data = action.item;
      if(data) {
        state.cur_ondata = data.ondata.toString(); // 保存当前查询日期 eg：2022-x-y
